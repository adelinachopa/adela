...
class Player(Entity):
    def __init__(self, x, y, color, speed, jump, controls, image_path=None, width=None, height=None):
        ...
        self.player_id = None  # идентификатор персонажа: "adelina" или "anya"
    ...
    def collide(self, dx, dy, platforms):
        for platform in platforms:
            if self.rect.colliderect(platform.rect):
                # Разделяем обработку по осям: сначала горизонтальная, потом вертикальная
                # Это предотвращает некорректную позицию при диагональных коллизиях
                if dx > 0:  # вправо
...
class DoorExit(Entity):
    ...
    def update(self, players, collected_items=0, total_items=0):
        ...
        for player in players:
            # Определяем имя персонажа по player_id (устанавливается при создании уровня)
            if hasattr(player, 'player_id'):
                if player.player_id == "adelina":
                    if self.activation_zone.colliderect(player.rect):
                        adelina_in_zone = True
                elif player.player_id == "anya":
                    if self.activation_zone.colliderect(player.rect):
                        anya_in_zone = True
