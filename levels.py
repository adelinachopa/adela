...
from setting import BLUE, RED, WHITE, GREEN
    ...
    def load_level(self, data):
        ...
        for plat in data.get('platforms', []):
            if isinstance(plat, dict):
                ...
                self.platforms.add(Platform(plat[0], plat[1], plat[2], plat[3], image_path=plat[4]))
        ...
        for p in data.get('players', []):
            ...
            # Устанавливаем идентификатор персонажа (из JSON или по порядку)
            player.player_id = p.get('id', f"player_{len(self.players) + 1}")
